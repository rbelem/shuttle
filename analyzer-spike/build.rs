...
//!
//! Source lists come from the tarball's own Sources.cmake (the build system
//! Luau itself uses), so nothing is hand-maintained per library:
...
const LIBS: &[&str] = &["Ast", "Config", "EqSat", "Analysis", "Compiler", "VM"];
...
fn main() {
    ...
    build
        ...
        .std("c++17")
        .warnings(false)
        // Must match the defines luau0-src uses, so a future link against
        // mlua's luau0-src static libs would be ABI-compatible.

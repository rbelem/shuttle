...
pub(crate) fn build_disk_image_with(
    ...
    if emits_sysupdate(image, disk_layout) {
        ...
        // #85: the core26 base ships neither systemd-bless-boot nor its
        // generator, so a counted boot could never be marked good (and a
        // hand-pulled bless service would exec a missing binary). Ship both
        // from the build host's systemd tooling when the base lacks them —
        // arch-gated, version-gated against the same #79 floor, and
        // fail-closed. UC22-era bases ship their own; staging is a no-op.
        staging::stage_bless_boot_binaries(runner, &root, arch)?;
        ...
        // #85/#79: the gate's post-condition — assessment emitted ⇒ the
...
";
    ...
    fn boot_complete_target_golden_text() {
        ...
        // (`systemd.special(7)`, `boot-complete.target`), including
        // DefaultDependencies=no — the #85 basic.target.wants/ pull-in
...
";
    ...
    fn boot_health_service_golden_text() {
        ...
        // DefaultDependencies=no + After=local-fs.target (#85): the implicit
        // After=basic.target (or an explicit After=default.target…) would
        // close basic → target → health → basic under the basic.target.wants/
...
";
    ...
    fn emit_boot_assessment_writes_units_and_gates_the_target() {
        ...
        // #85: the target IS pulled into every boot transaction via a
        // basic.target.wants/ link — factory boots get assessed too. A Wants=
        // link keeps a failed health check from failing the boot itself.
        let pull_in = root.path().join(BOOT_COMPLETE_PULL_IN_PATH);
        let meta = std::fs::symlink_metadata(&pull_in)
            .unwrap_or_else(|e| panic!("pull-in link missing at {}: {e}", pull_in.display()));
        assert!(meta.file_type().is_symlink(), "pull-in is a symlink");
        assert_eq!(
            std::fs::read_link(&pull_in).unwrap(),
            Path::new("../boot-complete.target"),
            "relative symlink target"
        );

        // The bless service itself is still never enabled: on counted boots
        // the generator pulls it; on factory boots there is nothing to bless.
        assert!(
            !root
                .path()
                .join("etc/systemd/system/multi-user.target.wants")
        ...
        assert!(
            !root
                .path()
                .join("etc/systemd/system/basic.target.wants")
                .join(BLESS_BOOT_UNIT_NAME)
                .exists(),
            "no unconditional bless enablement: factory boots must not run the \
             helper against a missing boot counter (#85)"
        );
    ...
    #[test]
    fn bless_tooling_gate_fails_closed_when_the_tooling_is_missing() {
        // #85/#79: assessment emitted, bless tooling absent (the measured
        // UC26 shape) ⇒ the build must refuse to pack the image.
        let root = tempfile::tempdir().unwrap();
        let err = assert_bless_boot_tooling_staged(root.path())
            .expect_err("an assessment without bless tooling must fail closed");
        let message = err.to_string();
        assert!(
            message.contains("boot assessment is emitted"),
            "names the condition: {message}"
        );
        assert!(
            message.contains("usr/lib/systemd/systemd-bless-boot"),
            "names the missing tooling: {message}"
        );
    }

    #[test]
    fn bless_tooling_gate_passes_when_both_binaries_are_staged() {
        let root = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(root.path().join("usr/lib/systemd/system-generators")).unwrap();
        std::fs::write(
            root.path().join("usr/lib/systemd/systemd-bless-boot"),
            b"ELF",
        )
        .unwrap();
        std::fs::write(
        ...
        .unwrap();
        assert_bless_boot_tooling_staged(root.path())
            .expect("both binaries present ⇒ the gate holds");
    }
    ...
    fn base_systemd_shared_lib_reports_the_path_for_the_tooling_rpath() {
        ...
        let (major, path) = base_systemd_shared_lib(root.path()).expect("the staged lib is found");
        assert_eq!(major, 259);
        assert_eq!(
            path.strip_prefix(root.path()).unwrap(),
            Path::new("usr/lib/x86_64-linux-gnu/systemd/libsystemd-shared-259.so"),
            "root-relative, so the RUNPATH can be the guest absolute path"
        );
        assert_eq!(base_systemd_major(root.path()), Some(259));
    }
    ...
    mod command_seam {
        ...
        struct E2eRunner {
            ...
            /// Whether the fake base rootfs ships the bless-boot tooling
            /// (#85). UC22-era bases do (the default); `without_bless_tooling`
            /// stages a UC26-shaped rootfs for the fail-closed e2e test.
            bless_tooling: bool,
        ...
        impl E2eRunner {
            ...
            /// Stage a systemd rootfs WITHOUT the bless-boot tooling — the
            /// measured UC26 shape (#85): a #79-passing systemd that still
            /// ships neither helper binary.
            fn without_bless_tooling(mut self) -> E2eRunner {
                self.bless_tooling = false;
                self
            }
        ...
        /// and — when the runner carries a systemd major — the base's
        /// systemd shared lib (the #79 gate's version source) plus the
        /// bless-boot tooling (#85: UC22-era bases ship both binaries, and
        /// the fast path of the staging step must observe exactly that).
        fn stage_rootfs_tree(dir: &str, systemd_major: Option<u32>, bless_tooling: bool) {
            ...
            if let Some(major) = systemd_major {
                ...
                if bless_tooling {
                    std::fs::write(
                        systemd_dir.join("systemd-bless-boot"),
                        b"\x7fELF-fake-bless-boot",
                    )
                    .unwrap();
                    let generators = systemd_dir.join("system-generators");
                    std::fs::create_dir_all(&generators).unwrap();
                    std::fs::write(
                        generators.join("systemd-bless-boot-generator"),
                        b"\x7fELF-fake-bless-boot-generator",
                    )
                    .unwrap();
                }
            }
                ...
                match program {
                    ...
                    "unsquashfs" => {
                        ...
                        if let Some(dir) = arg_after(argv, "-d") {
                            stage_rootfs_tree(dir, self.systemd_major, self.bless_tooling);
                        }
                        Ok(out(0, Vec::new()))
                    }
                    "dd" => {
        ...
        fn build_disk_image_with_ab_and_update_source_emits_sysupdate_units() {
            ...
            // the `boot-complete.target.requires/` link, and since #85 the
            // `basic.target.wants/` pull-in that puts the target into every
            // boot transaction.
            for rel in [
                BLESS_BOOT_UNIT_PATH,
                BOOT_COMPLETE_TARGET_PATH,
        ...
        #[test]
        fn build_disk_image_fails_closed_without_bless_tooling() {
            // #85: the measured UC26 shape — a #79-passing systemd whose base
            // ships neither systemd-bless-boot nor its generator, with no
            // build-host tooling to ship (the fake `which` answers empty).
            // The assessment must never be packed unbacked.
            let stub_dir = tempfile::tempdir().unwrap();
            let _path_guard = stub_disk_tool_path(stub_dir.path());
            let (_cache_dir, cache, digest) = cache_fixture();
            let image = ab_update_source_image("e2enobless", &digest);

            let runner = E2eRunner::new("amd64", &digest).without_bless_tooling();
            let mut lockfile = LockFile::empty();
            let output = tempfile::tempdir().unwrap();
            let err = build_disk_image_with(
                &runner,
            ...
            let message = err.to_string();
            assert!(
                message.contains("bless-boot tooling cannot be provided"),

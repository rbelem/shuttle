...
/// `etc/fstab` content for a state split. The persistent state partition
/// mounts at `/var/lib`; `/var` itself is a tmpfs so cache/log/tmp never
/// grow the persistent surface (ADR-0023 §2). Both carry `nofail` when
/// declared `x-systemd.requires-mounts-for`, so a bad state partition
/// cannot wedge early boot in the emergency shell.
pub(crate) fn fstab_content(split: &StateSplit) -> String {
    ...
    out.push_str(&format!("tmpfs {VAR_MOUNT} tmpfs mode=0755,nosuid,nodev\n"));
    out
}
...
/// skeleton (`run`, `tmp`, `cache`, `log`) on the tmpfs at boot.
pub(crate) fn var_tmpfiles_content() -> String {
    let mut out = String::from("# Generated by shuttle — do not edit.\n");
    out.push_str(&format!("d {} 0755 root root -\n", VAR_MOUNT));
...
/// Emit the `/var` split into the staged rootfs: `etc/fstab`, the state
/// tmpfiles, and the volatile-`/var` tmpfiles. Written before root
/// populate + dm-verity — the same write-before-hash constraint as the
/// manifest and app runtime — so `/etc/fstab` lands inside the hashed tree.
pub(crate) fn emit_state_split(root: &Path, split: &StateSplit) -> miette::Result<()> {
    crate::emit::write_staged_file(root, Path::new(FSTAB_PATH), &fstab_content(split))?;
    crate::emit::write_staged_file(root, Path::new(STATE_TMPFILES_PATH), &state_tmpfiles_content())?;
    crate::emit::write_staged_file(root, Path::new(VAR_TMPFILES_PATH), &var_tmpfiles_content())?;
    eprintln!(
        "  ✓ /var split: state on PARTLABEL={} → {STATE_MOUNT}, /var tmpfs + tmpfiles",
        split.partlabel
    );
    Ok(())
}

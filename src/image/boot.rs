...
/// Boot-time stranded-slot recovery oneshot (issue #86). A killed
/// sysupdate install can leave the target slot carrying the new version
/// label with no matching UKI on the ESP; the next update then finds no
/// writable target (systemd 249 has no vacuum verb). This unit reclaims
/// such slots at boot, BEFORE the next install can look for one.
pub(crate) const SLOT_RECOVERY_UNIT_NAME: &str = "shuttle-slot-recovery.service";
pub(crate) const SLOT_RECOVERY_UNIT_PATH: &str = "etc/systemd/system/shuttle-slot-recovery.service";

/// ExecStart program for the stranded-slot recovery oneshot. Pinned
/// absolutely to `/{SHUTTLE_BIN_PATH}` for the same reason as
/// [`BOOT_HEALTH_EXEC`] (issue #81: the binary is embedded at that exact
/// path in every image the build produces); the `--esp-mount` value is
/// appended by [`slot_recovery_service_content`] from the image's own
/// ESP declaration, so the guest never guesses a mountpoint.
pub(crate) const RECOVER_SLOTS_EXEC: &str = "/usr/bin/shuttle runtime recover-slots";
...
/// Render `shuttle-slot-recovery.service` (issue #86).
///
/// A `Type=oneshot` with `RemainAfterExit=yes`, enabled into
/// `multi-user.target` and ordered `Before=systemd-sysupdate.service`:
/// recovery must complete before the next install transaction looks for a
/// writable slot. It needs the disk (partition labels) and the ESP
/// (installed UKIs) visible, hence `After=local-fs.target` +
/// `RequiresMountsFor=<esp>`. The command is conservative by construction
/// (see `crate::slot_recovery`): it relabels only when the evidence proves
/// the slot was never bootable, and every refusal is a named no-op — a
/// boot never wedges on this unit.
pub(crate) fn slot_recovery_service_content(esp_mount: &str) -> String {
    format!(
        "\
# Generated by shuttle — do not edit.
[Unit]
Description=shuttle: reclaim sysupdate slots stranded mid-install (issue #86)
# The strand evidence lives on the disk (partition labels) and the ESP
# (installed UKIs); both must be visible before the check runs.
After=local-fs.target
RequiresMountsFor={esp_mount}
# Reclaim BEFORE the next install searches for a writable slot — a
# stranded label would make that search fail (or silently no-op).
Before={SYSUPDATE_SERVICE_NAME}

[Service]
Type=oneshot
RemainAfterExit=yes
ExecStart={RECOVER_SLOTS_EXEC} --esp-mount {esp_mount}
...
///
/// The same gate also emits the stranded-slot recovery oneshot (issue
/// #86) into `multi-user.target`: recovery only makes sense where the
/// transfer definitions exist, so one predicate drives all three and none
...
pub(crate) fn emit_sysupdate_units(root: &Path, esp_mount: Option<&str>) -> miette::Result<()> {
    ...
    // Issue #86: the boot-time stranded-slot reclaim, ordered before the
    // update service it preconditions.
    crate::emit::write_unit(
        root,
        Path::new(SLOT_RECOVERY_UNIT_PATH),
        &slot_recovery_service_content(esp_mount.unwrap_or("/boot")),
    )?;
    crate::emit::enable_unit(root, "multi-user.target", SLOT_RECOVERY_UNIT_NAME)?;
    eprintln!("  ✓ {SYSUPDATE_TIMER_NAME} + {SYSUPDATE_SERVICE_NAME} emitted (ADR-0024 §2)");
    eprintln!("  ✓ {SLOT_RECOVERY_UNIT_NAME} emitted, Before={SYSUPDATE_SERVICE_NAME} (issue #86)");
    Ok(())
}
...
pub(crate) fn locate_kernel_payload(
    ...
    // Issue #74: recognize the Raspberry Pi payload shape BEFORE the
...
/// The Raspberry Pi kernel-snap payload shape (issue #74): raw components
/// at the snap root under their Pi spellings — `kernel.img` (a
/// gzip-compressed ARM64 Image with the EFI stub compiled in) and
/// `initrd.img` (a compressed initramfs), alongside `modules/<kver>/`,

...
/// of the initial boot transaction. The thing that pulls it in is
/// `systemd-bless-boot.service`'s `Requires=`: the stock
/// `systemd-bless-boot-generator` adds that service to `basic.target` when
/// boot counting is in effect, and its `Requires=` then pulls the target,
/// whose `requires/` link pulls this health check. Note the direction: the
/// health unit's `Before=`/`RequiredBy=` do NOT pull the target in — a
/// `Requires=` is forward-only, so nothing propagates backwards from a target
/// to its requirers. `systemd-bless-boot.service` is not enabled either; the
/// generator is what reaches it.
///

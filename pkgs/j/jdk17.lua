-- jdk17: Eclipse Temurin 17 LTS JDK — the Java development kit as a
-- prebuilt binary port (Adoptium jdk-17.0.20.1+1, the 17.0.20.1
-- security-update build over the 17.0.20 line, JAVA_VERSION_DATE
-- 2026-08-18; Adoptium API latest-GA for
-- version=[17],x64/linux/jdk/hotspot resolved 2026-09-20). sha256
-- 3808d1d15e3ec6bd5b84057fb5d84c33d8a1536a258146bcea2e603fc726e08e
-- cross-checked three ways: api.adoptium.net asset checksum, the
-- published .sha256.txt, and the fetched bytes — all three agree.
-- https://github.com/adoptium/temurin17-binaries (the GitHub release
...
--     jdk17-corretto follow the same shape with their own pins
--
...
-- strand lib/modules (go.lua issue #46 gap verbatim, at 28x
-- scale — 17 carries no jwebserver, that tool arrives in JDK 18).
-- Each tool gets a two-line sh launcher AT usr/<tool>
-- (exec $(dirname $0)/bin/<tool>) so the assembly root is the
-- prefix: the whole JDK hardlinks beside every app (hardlinks, not
-- copies — one store blob set serves all 28 assemblies).
--
-- JAVA_HOME is NOT hardcoded here — a port cannot know which pod
-- will host it. The jdk17-pod declares it per ADR-0030 (generation-
...
-- `<pod-root>/current/../apps/jdk17/usr` (current →
...
return {
    default = snap {
        name = "jdk17",
        version = "17.0.20.1+1",
        summary = "Temurin 17 LTS JDK — binary build for per-version JVM pods",
        description = [[
            Eclipse Temurin 17 LTS (jdk-17.0.20.1+1): the full JDK —

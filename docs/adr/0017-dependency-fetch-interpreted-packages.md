...
## Addendum (2026-09-09): cargo and go resolvers

The mechanism proved ecosystem-agnostic exactly as designed — the "gem/cargo time" second redesign the rejected alternative anticipated did not happen; resolvers are additive. Landed: **cargo** (`deps = { cargo = { lock = "Cargo.lock" } }`, crates.io fetches verified against lockfile checksums, vendored-directory layout + `CARGO_NET_OFFLINE=true`, commit `b7e0b7b`) and **go** (`deps = { go = { mods = "go.mod" } }`, GOPROXY fetches verified against go.sum, module-cache layout + `GOFLAGS=-mod=vendor`, ticket #40). Same closure blob, lockfile pin, tamper refusal, and offline-mount guarantees as npm/pip. Known gap recorded by the cargo landing: pod-built toolchain payloads carry store-exec launcher wrappers (#12) that cannot serve as merged-prefix `build_deps` toolchains — a wrapper-aware prefix build is the follow-up.

## Addendum (2026-09-14): wrapper-aware merged-prefix staging (issue #90)

The recorded gap is closed. `materialize_merged_prefix` is now wrapper-aware: payloads staged into the merged build prefix carry the pod-runtime launcher wrappers (issues #9/#10/#13 — exec targets under `$PODROOT/active/extensions/<pkg>/usr/…`, the `usr/usr` doubling, and pod-store blob paths), which resolve nowhere in the prefix. At staging time the prefix re-points them deterministically — extension-tree segments dropped (the payload root merges at the prefix root), blob-path execs replaced by the preserved `.real` sibling resolved from the wrapper's own directory — and fails closed, naming the wrapper and its owning payload, when an interpreter or exec target resolves to neither the merged payloads nor a declared `requires`. Same discipline for the build-time app pass: a script command with no declared `interpreter` resolves its shebang interpreter against the payload set (perltidy's `#!/usr/bin/perl`) and lands the #9 farm-executable wrapper.

## Addendum (2026-09-21): recipe-shipped lockfiles (`recipe/` lock paths)

A `deps.*.lock` value prefixed `recipe/` (go's `sum` included, whose sibling default keeps the prefix) resolves against the package recipe directory — the dir holding the package's `init.lua`, or of the single `<name>.lua` — fail-closed: no source-tree fallback, `..` rejected, and a missing file is the named-diagnostic error. Plain values keep source-tree-first resolution with the recipe dir as fallback, so existing declarations are unchanged. Whatever bytes win feed the `deps_hash` pin exactly as before, and a lock resolved from the recipe dir records its sha256 as `lock_sha256` beside the pin — the audit trail for pool packages whose upstreams ship no lockfile (pdf-inspector, agentmemory).
